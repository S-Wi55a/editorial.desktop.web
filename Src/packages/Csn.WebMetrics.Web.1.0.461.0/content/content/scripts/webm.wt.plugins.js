(function () {
    webmTrafficSource = {
        TrafficSourceAttribution: function (isBrandRelated, tag, windowP, documentP) {
            ...
            that.isCsNetwork = function () {
                ...
                var hostnameLCase = document.location.hostname.toLowerCase();
                if (that.rhost.indexOf(rd) == -1) {
                    if (rd) {
                        for (var i = 0, csnl = csNetwork.length; i < csnl; i++) {
                            if (hostnameLCase.indexOf(csNetwork[i].toLowerCase()) >= 0) {
                                continue;
                            }
        ...
        trackReferrals: function (isBrandRelated) {
			...
			* --------------------- */
            ...
            var csNetwork = ["bikesales", "boatsales", "caravancampingsales", "quicksales", "redbook", "motoring", "trucksales", "carpoint", "bikepoint", "boatpoint", "farmmachinerysales", "constructionsales", "livemarket", "discountnewcars", "carsales"];
            ...
            function isCarSalesNetwork() {
                ...
                for (var i = 0, csnl = csNetwork.length; i < csnl; i++) {
                    if (hostname.indexOf(csNetwork[i].toLowerCase()) >= 0) {
                        continue;
                    }
    ...
    adBlockChecker = {
        check: function () {
            // confirm that ads.js exists on the page
            if (document.getElementById("cs_ads_det")) {
                // delay check to make sure everything is finished loading
                if (window.loadedAds === undefined) {
                    // ad blocker detected
                    Webtrends.multiTrack({
                        argsa: ["WT.dl", "92", "WT.z_adblock", "Detected"]
                    });
                } else {
                    // ad blocker not detected
                    Webtrends.multiTrack({
                        argsa: ["WT.dl", "92", "WT.z_adblock", "Not Detected"]
                    });
                }
            } else {
                // element with ID=blocker_test not on the page, so cannot check for an ad blocker
                Webtrends.multiTrack({
                    argsa: ["WT.dl", "92", "WT.z_adblock", "Not Checked"]
                });
            }
        }
    };
    var fvd = {
        ...
        firstVisitDays: function (dcs, options, window, document) {
    ...
    allPlugins = {
        doWork: function (dcs, options) {
            ...
            try {
                if (window.runAdBlockerPlugin) {
                    adBlockChecker.check();
                }
            } catch (e) { }
            try {
                if ((fvd) && (typeof (fvd.firstVisitDays) == 'function')) {
                    fvd.firstVisitDays(dcs, options);
                }
            } catch (e) { }
        }
    };
