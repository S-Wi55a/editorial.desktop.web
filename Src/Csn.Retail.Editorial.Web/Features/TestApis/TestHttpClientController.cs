...
namespace Csn.Retail.Editorial.Web.Features.TestApis
{
    ...
    {
        private readonly IRyvussServiceHttpClient _ryvussDataService;
        private readonly IMapper _mapper;
        private readonly IHttpClientFactory _httpClientFactory;

        public TestHttpClientController(IRyvussServiceHttpClient ryvussDataService,
                                IMapper mapper,
        ...
        [HttpGet]
        [Route("editorial/api/test/http/carousels")]
        ...
        {
            var client = _httpClientFactory.GetOrCreate("Showrooms-Api-Test");
    ...
    public interface IRyvussHttpClientProxy
    {
        Task<T> GetAsync<T>(EditorialRyvussInput input);
    }

    [AutoBind]
    public class RyvussHttpClientProxy : IRyvussHttpClientProxy
    {
        private readonly IHttpClientFactory _httpClientFactory;

        public RyvussHttpClientProxy(IHttpClientFactory httpClientFactory)
        {
            _httpClientFactory = httpClientFactory;
        }
        ...
        {
            var client = _httpClientFactory.GetOrCreate("Ryvuss-Api-Test");
    ...
    public interface IRyvussServiceHttpClient
    {
    ...
    [AutoBind]
    public class RyvussServiceHttpClient : IRyvussServiceHttpClient
    {
        private readonly ITenantProvider<TenantInfo> _tenantProvider;
        private readonly IRyvussHttpClientProxy _ryvussHttpClientProxy;

        public RyvussServiceHttpClient(ITenantProvider<TenantInfo> tenantProvider, IRyvussHttpClientProxy ryvussHttpClientProxy)
        {
            _tenantProvider = tenantProvider;
            _ryvussHttpClientProxy = ryvussHttpClientProxy;
        }
        ...
        {
            return _ryvussHttpClientProxy.GetAsync<RyvussNavResultDto>(new EditorialRyvussInput
            {
                Query = query,
        ...
        private Task<RyvussNavResultDto> GetRyvusProxyResults(string query, List<string> postProcessors, bool includeResults = false, string sort = "", int offset = 0)
        {
            ...
            return _ryvussHttpClientProxy.GetAsync<RyvussNavResultDto>(ryvusInput);
        }
    }
}