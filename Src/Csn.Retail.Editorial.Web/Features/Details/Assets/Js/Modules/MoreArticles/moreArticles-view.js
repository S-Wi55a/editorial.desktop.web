...
const container = (data) => {
    ...
    const filterView = (str, filter, index) => {
        return `<div class="more-articles__filter${index === 0 ? " more-articles__filter--active" : ""}" data-webm-clickvalue="view-${hyphenateNonAlphaNumeric(filter.title)}" data-more-articles-query="${moreArticlesPath}${filter.uri}">${filter.title}</div>`;
    }
            ...
            <div class="container">
                ...
                <div class="more-articles__nav">
                    ...
                    <button class="more-articles__nav-button more-articles__nav-button--next" data-webm-clickvalue="next"
                        data-more-articles-query="${moreArticlesPath}${data.filters[0].uri || ''}">Next</button>
                </div>
            </div>
        </div>
        `;
